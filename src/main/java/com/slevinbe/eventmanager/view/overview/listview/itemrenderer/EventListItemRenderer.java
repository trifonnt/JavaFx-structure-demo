	...
	private class EventCell extends ListCell<EventVO> {
		...
		public void updateItem(EventVO item, boolean empty) {
			...
			if (item != null) {
				...
				Label nameLabel = LabelBuilder.create().text(item.getName().toUpperCase()).layoutX(10).layoutY(5)
						.styleClass("eventItemHeader").build();
				...
				Label dateLabel = LabelBuilder.create().text(startTime + " - " + endTime).layoutX(10)
						.styleClass("eventItemDateLabel").build();
				dateLabel.layoutYProperty().bind(nameLabel.layoutYProperty().add(nameLabel.heightProperty()).add(5));

				Label locationLabel = LabelBuilder.create().text(" @ " + item.getLocation())
						.styleClass("eventItemLocationLabel").build();
				...
				Label descriptionLabel = LabelBuilder.create().text(item.getDescription()).layoutX(10).wrapText(true)
						.styleClass("eventItemDescriptionLabel").build();
				descriptionLabel.layoutYProperty()
						.bind(dateLabel.layoutYProperty().add(dateLabel.heightProperty().add(5)));
				descriptionLabel.prefWidthProperty().bind(getListView().widthProperty().subtract(40));

				Pane container = PaneBuilder.create()
						.children(nameLabel, locationLabel, dateLabel, descriptionLabel)
						.layoutX(10).prefHeight(120).styleClass("eventListItemContainer").build();
