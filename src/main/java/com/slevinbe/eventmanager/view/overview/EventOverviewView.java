...
public class EventOverviewView implements IEventOverviewView {
	...
	public Pane getView() {
		if (view == null) {
			view = PaneBuilder.create().padding(new Insets(5)).children(eventList = eventListView.getView()).build();
			eventList.prefWidthProperty().bind(view.widthProperty());
			eventList.prefHeightProperty().bind(view.heightProperty());

			hideEventListTransition = FadeTransitionBuilder.create().node(eventList).byValue(-1)
					.duration(new Duration(600)).interpolator(Interpolator.EASE_OUT).autoReverse(true).build();

			showEventListTransition = FadeTransitionBuilder.create().node(eventList).byValue(1)
					.duration(new Duration(600)).interpolator(Interpolator.EASE_OUT).autoReverse(true).build();
	...
	private void initDetailsPane() {
		if (detailsPane == null) {
			...
			hideDetailsPaneTransition = TranslateTransitionBuilder.create().node(detailsPane).toX(view.getWidth())
					.duration(new Duration(300)).interpolator(Interpolator.LINEAR).build();

			showDetailsPaneTransition = TranslateTransitionBuilder.create().node(detailsPane).fromX(view.getWidth())
					.toX(view.widthProperty().multiply(.025).doubleValue()).duration(new Duration(300))
